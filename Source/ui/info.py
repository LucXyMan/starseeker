...
class _Speed(_String):
    ...
    def update(self):
        ...
        self.string = u"Speed:{}".format(
            _const.SPEED_TEXTS[_inventories.Utils.get_speed()])
...
class _Player(_String):
    ...
    def update(self):
        ...
        self.string = u"Player:{name}".format(
            name=__units.get_player(_inventories.Utils.get_player()).name)
        _layouter.Menu.set_speed(self)
...
class _Next(_String):
    ...
    def update(self):
        u"""文字列更新。
        """
        level = _inventories.Utils.get_endless()+1
...
class _Time(_String):
    ...
    def update(self):
        ...
        one_hour = 3600
        one_minute = 60
        hour, minute = divmod(_inventories.Time.get(), one_hour)
...
class _Score(_String):
    ...
    def update(self):
        ...
        self.string = u"SP:{sp} ★:{wins}".format(
            sp=_inventories.SP.get(), wins=_inventories.Level.get_wins())
        _layouter.Menu.set_level(self)
...
class Info(object):
    ...
    def eliminate(self):
        u"""全ての内容物をkill。
        """
        for i, content in enumerate(self.__strings):
            if content:
                content.kill()
    ...
    def has_info(self):
        u"""情報表示取得。
        """
        return bool(self.__strings[self.__INFO_SLOT])
