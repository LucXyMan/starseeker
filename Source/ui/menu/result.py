...
class Result(_content.Control):
    ...
    def __init__(self):
        ...
        if _inventories.Utils.get_endless() % _const.ENDLESS_INTRVAL == 0:
            reward = __levels.get_reward()
            if reward != 0:
                _inventories.Items.on(reward-1)
    ...
    def _update_info(self):
        ...
        if window.items:
            ...
            got = _inventories.Card.get_consume(self.__got)
            __sprites.Info.send(
                item.info if icon.is_front else
                u"決定キーで{type}カード取得/{got}SP消費".
                format(type=item.type, got=got))
    ...
    def decision(self):
        ...
        if window.items:
            ...
            if (
                ...
                _inventories.SP.buy_card(icon.number, self.__got)
            ):
                sound.SE.play("get")
                icon.is_front = True
                self.__got += 1
