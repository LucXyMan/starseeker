...
class Item(_Icon):
    ...
    def update(self):
        ...
        ep_value = _inventories.SP.get()
        utils_value = _inventories.Utils.get_value()
        level_value = _inventories.Level.get_value()
        items_value = _inventories.Items.get_value()
        ...
        if self._old != (
            ...
            has = _inventories.Items.has(self._number-1)
            chest = __equips.get_chest(item.is_locked, item.rank).icon
            is_item_equipable = (
                _inventories.Skill.
                is_item_equippable(__equips.get(self._number)))
            image = ((
                chest if not item.is_sealed else
                _image.get_dull(chest)) if not has else
                item.icon if is_item_equipable else _image.get_dull(item.icon))
            ...
            subscript = (
                ...
                not item.is_sealed and _inventories.SP.is_buyable(item) else
                "")
