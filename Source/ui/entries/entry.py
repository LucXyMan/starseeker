...
class Entry(_content.Content):
    ...
    _LEVEL_SELECT_TEXT = (
        ...
        _content.Content._PLAYER_SELECT_TEXT)

    def __init__(self):
        u"""コンストラクタ。
        """
        import ui.menu.customize as __customize
        ...
        class _RivalStatus(_sprites.Window):
            ...
            def __init__(self, content, groups=None):
                ...
                super(_RivalStatus, self).__init__(
                    (0, 0),
                    __pygame.Surface((col*_const.GRID, row*_const.GRID)),
                    groups)
    ...
    def __change_player(self, value):
        ...
        player = __inventories.Utils.get_player()
        __inventories.Utils.set_player((player+value) % _const.PLEYERS)
        if __inventories.Utils.get_player() != player:
