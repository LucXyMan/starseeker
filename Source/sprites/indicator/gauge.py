...
class Gauge(_general.Indicator):
    u"""ゲージスプライト。
    """
    __ALPHA = None
    __GAUGE_SIZE = 42, 4
    _BACK_COLORS = _const.BLACK, _const.GRAY
    _LAYER = 0
    ...
    def update(self):
        ...
        def __set_layer():
            u"""レイヤー設定。
            """
            current_layer = self._caster.get_layer()
...
class Life(Gauge):
    ...
    def update(self):
        ...
        def __set_parameter():
            ...
            if life != self._old:
                scale = int(
                    (self._caster.life/float(self.__GAUGE_LIMIT)) *
                    (len(self._images)-1))
                self._dest = (
                    scale if scale < len(self._images) else
                    len(self._images)-1)
                self._text = str(
                    life if life < self.__LIFE_LIMIT else self.__LIFE_LIMIT)
                self._color = (
...
class Charge(Gauge):
    ...
    def _get_images(self):
        ...
        return self._get_gauge_images(
            ...
            (_pygame.Color(_const.BLACK), _pygame.Color(_const.GRAY)), 1)
    ...
    def update(self):
        ...
        self._rise_and_fall()
        if (
            hasattr(self._caster, "frozen_time") and
            self._caster.frozen_time != 0
        ):
            self.image = _image.get_clear(self.image)
        else:
            self.image = self._images[self._scale]
...
class Frozen(Gauge):
    ...
    def __init__(self, caster, groups=None):
        u"""コンストラクタ。
        """
        super(Frozen, self).__init__(caster, groups)
        self._text = "Frozen"
    ...
    def update(self):
        ...
        super(Frozen, self).update()
        __set_parameter()
        self._rise_and_fall()
        self.image = self._images[self._scale]
        self._set_string()
        if self._caster.frozen_time == 0:
            self.image = _image.get_clear(self.image)
        self.rect = self.image.get_rect()
        _layouter.Game.set_charge_gauge(self, self._caster)
