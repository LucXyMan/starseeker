...
def init():
    ...
    インベントリインスタンスを一つだけ作成。
    """
    Inventory.save_location = _os.path.join(
        _os.path.expanduser("~"), ".config", "starseeker")
    Inventory()
...
class Inventory(object):
    ...
    __ITEMS_DATA_SIZE = 5
    __CARDS_DATA_SIZE = 4
    __EQUIP_DATA_SIZE = _const.PLAYER_NUMBER
    __SKILL_DATA_SIZE = _const.PLAYER_NUMBER >> 1
    __DECK_DATA_SIZE = _const.PLAYER_NUMBER << 2
    ...
    def __init__(self):
        ...
        __items: 取得アイテム状態。
        __cards: 最大３枚の取得済カード。
        __equip: プレイヤー８人分の装備・スキル・デッキの状態。
        __skill: プレイヤー８人分のスキル状態。
        __deck: プレイヤー８人分のデッキ状態。最大３のカード枚数。
        """
        ...
        def __init_starter_deck(*cards):
            ...
            for card in cards:
                ...
                for i in range(_const.PLAYER_NUMBER):
                    Inventory.__deck.set(
                        i*len(Inventory.__deck)/_const.PLAYER_NUMBER+card, 1)
        Inventory.__time = 0
        Inventory.__sp = 100
        ...
        if not hasattr(self, "__CardsContainer"):
            Inventory.__CardsContainer = __bitflag.TwoBitNumber
        Inventory.__cards = self.__CardsContainer([0]*self.__CARDS_DATA_SIZE)
        ...
        __init_starter_deck(0, 5, 10, 16, 33, 46, 53, 57)
    ...
    def __get_hash(cls, values):
        ...
        algorithm.update(reduce(
            __operator.add, (str(value) for value in values)))
    ...
    def load(cls):
        ...
        if _os.path.exists(filename):
            ...
            cards = cls.__CardsContainer(_struct.unpack(
    ...
    def save(cls, filename=__SAVE_FILE):
        ...
        _packer.pack(_os.path.join(cls.save_location, filename), (
            ...
            cls.__get_hash((
                cls.__time, cls.__sp, cls.__utils, cls.__level, cls.__items,
                cls.__cards, cls.__equip, cls.__skill, cls.__deck))))
