...
class Falling(_Operatable):
    u"""落下ピース。
    """
    __slots__ = (
        "__is_commanded", "__is_rested", "__is_t_spin",
    ...
    # ---- Rotation ----
    def __rotate(self, field, clock_wise):
        u"""回転処理。
        """
        ...
        if not self.__is_rested:
            ...
            for pos in __get_rotation_order(
    ...
    def rotate(self, field, clock_wise=True):
        u"""通常使用する回転処理。
        """
    ...
    def detect_rotation(self, field, clock_wise=True):
        u"""AI用回転処理。
        """
        old_state = self.state
        if (
            not self.__is_rotatable or
            not self.__rotate(field, clock_wise)
        ):
