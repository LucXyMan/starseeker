...
class Entry(_entry.Entry):
    ...
    def __update_notice(self):
        ...
        if self.__old != (rival, playable):
            level_select = u"#"+self._LEVEL_SELECT_TEXT.format(level=u"レベル")
            if playable:
                __sprites.Notice.notify(
                    _units.get_player(_levels.get_rival()).notice+level_select)
            else:
                __sprites.Notice.notify(u"???/まだ戦えません"+level_select)
            self.__old = rival, playable

    # ---- Up and Down ----
    def __change_level(self, value):
        u"""レベル変更。
        """
        limit = 3
